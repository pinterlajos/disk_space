...
#define DISK_SPACE_PLUGIN(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), disk_space_plugin_get_type(), \
                              DiskSpacePlugin))

struct _DiskSpacePlugin {
  GObject parent_instance;
};

G_DEFINE_TYPE(DiskSpacePlugin, disk_space_plugin, g_object_get_type())
...
static void disk_space_plugin_handle_method_call(
    DiskSpacePlugin* self,
...
static void disk_space_plugin_dispose(GObject* object) {
  G_OBJECT_CLASS(disk_space_plugin_parent_class)->dispose(object);
}

static void disk_space_plugin_class_init(DiskSpacePluginClass* klass) {
  G_OBJECT_CLASS(klass)->dispose = disk_space_plugin_dispose;
}

static void disk_space_plugin_init(DiskSpacePlugin* self) {}

static void method_call_cb(FlMethodChannel* channel, FlMethodCall* method_call,
                           gpointer user_data) {
  DiskSpacePlugin* plugin = DISK_SPACE_PLUGIN(user_data);
  disk_space_plugin_handle_method_call(plugin, method_call);
}

void disk_space_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
  DiskSpacePlugin* plugin = DISK_SPACE_PLUGIN(
      g_object_new(disk_space_plugin_get_type(), nullptr));
